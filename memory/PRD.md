...
- **Phase 14 (2026-08-13):** **New money page `/muk-buddy-reusable-bag/` shipped** — SEO destination for "reusable shop vac bag" and "reusable wet dry vac bag" terms per Jed's PAGE-SPEC-v1.
  - New file: `frontend/src/pages/MukBuddyReusableBagPage.jsx`. Route wired in `App.js` (both `/muk-buddy-reusable-bag` and trailing-slash variant).
  - Header nav updated: new "Reusable Bag" link (`data-testid="nav-reusable-bag"`) added to `Header.jsx` as the first item — gives site-wide internal linking. Existing nav hash anchors converted from `#foo` to `/#foo` so they still resolve from the new page.
  - Prerender: `/muk-buddy-reusable-bag` added to `scripts/prerender.js` ROUTES. Emits `build/muk-buddy-reusable-bag/index.html` (67KB) with all copy, schema, and canonical baked in.
  - Sitemap: entry added at priority 0.9 (higher than blog index at 0.8) via `scripts/build-blog.js`.
  - Compliance verified in prerendered HTML: 0 "patented" / 0 "bag tax" / 0 "free shipping" / 0 "limited time" / 0 "order today". "Patent Pending" used throughout. No competitor names. Washing mentioned only in Section 6 (Care).
  - Schema: emits `Product` + `FAQPage` + `BreadcrumbList` in a single `@graph`. **NO `aggregateRating`** (would require verifiable reviews — manual-action risk per Google).
  - CTAs (hero + closing) point to `https://thefloorlord.com/checkout/?add-to-cart=6541&quantity=1` and are wrapped with `appendAttribution()` so gclid/wbraid/fbclid propagate to WooCommerce.
  - Verified: `<meta name="robots" content="index, follow, max-image-preview:large">`, canonical self-references, exactly 1 `<h1>`, 58-char title, 165-char meta description.
- **Phase 13 (2026-07-01):** SEO audit batch #2 shipped. All 6 items verified in prerendered HTML.
  1. `/ads` emits `<meta name="robots" content="noindex, follow">` (verified in `build/ads/index.html`); the route is also absent from `sitemap.xml`. Meta tag is baked in via `PageHead.jsx` → `react-helmet-async` → Puppeteer prerender, so a server-header wipe on deploy cannot break it.
  2. SSR count-up fix confirmed: `CountUp.jsx` initializes `useState(to)` so prerendered HTML carries the real values ($300+, $200+, $850, $1,000, $12,000, $2,400, 500+, etc.). The only remaining `$0` strings in the SSR are intentional marketing copy in Problem.jsx (`"$0 recurring filters · $0 new motors"`) — a positive selling point about Muk Buddy's zero recurring cost, NOT a broken counter.
  3. Homepage → blog links: 5 in-body `<a href>` links (best-shop-vac-bags, disposable-vs-reusable, lost-suction, drywall-clogs, expensive-bags) inside `WhatIsMukBuddy.jsx` "Read More" section on `/`. Passes link equity to top money posts.
  4. `WhatIsMukBuddy.jsx` component added — self-contained 155-word AI-citation definition paragraph placed AFTER Hero, BEFORE BagsVsMukBuddy on `/`. Includes 2-chamber design, patent-pending, brand fitment, $99 price, Made in USA, 28-year contractor origin.
  5. Real HTML `<table data-testid="compatibility-table">` — Shop-Vac / RIDGID / DeWalt / Craftsman / Stanley × gallon ranges (12–20) × common models. Sits inside `WhatIsMukBuddy.jsx` under the definition paragraph. Not an image.
  6. `scripts/build-blog.js:414` — `ItemList` JSON-LD block preserved. Emits schema.org ItemList when a post has `item_list:` frontmatter (verified live on `/blog/best-shop-vac-bags/index.html`). **Do NOT remove this block.**
  Also: `/ads` Lighthouse TBT fix — added `LazyBackgroundVideo` + `LazyForegroundVideo` (IntersectionObserver, `rootMargin: 400px`). Prerendered `/ads/index.html` has 0 upfront references to `contractor-vac*` or `mascotAnimation` (video bytes deferred to viewport).
