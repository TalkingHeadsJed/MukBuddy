...
- **Phase 13 (2026-07-01):** SEO audit batch #2 shipped.
  - Created `frontend/src/components/sections/WhatIsMukBuddy.jsx` — a self-contained "What is Muk Buddy?" block placed AFTER Hero, BEFORE BagsVsMukBuddy on `/`. Contains: (a) 155-word AI-citation definition paragraph mentioning the 2-chamber design, patent-pending, brand fitment, and $99 price; (b) HTML compatibility table listing Shop-Vac / RIDGID / DeWalt / Craftsman / Stanley with gallon ranges + common models (`data-testid="compatibility-table"`); (c) 4 contextual homepage → blog links passing link equity to the top-money-post pages (best-shop-vac-bags, disposable-vs-reusable, lost-suction, drywall-clogs).
  - Verified `ItemList` JSON-LD schema is compiling correctly on roundup-style posts (`/blog/best-shop-vac-bags/`) via `build-blog.js` line 414 — driven off the `item_list:` frontmatter field.
  - Verified `/ads` route still emits `<meta name="robots" content="noindex, follow">` post-prerender.
  - `/ads` Lighthouse TBT fix: Added `LazyBackgroundVideo` + `LazyForegroundVideo` helpers using IntersectionObserver (`rootMargin: 400px`). The two decorative WebM background videos on AirflowDemo/SavingsCalc + the mascot demo video no longer preload or attach `<source>` tags until scrolled near the viewport. Prerendered `/ads/index.html` now contains 0 references to `contractor-vac*` or `mascotAnimation`, so first-paint mobile crawlers pay ~0 KB for video bytes. Meaningful TBT savings expected.
